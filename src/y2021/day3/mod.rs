...
pub fn calculat_gama(list: Vec<&str>) -> String {
    ...
    acc.iter().map(|digit| if digit > &(&list.len()/2) {"1"} else {"0"}).collect::<Vec<&str>>().join("")
}
...
fn filter_oxygen(list: Vec<&str>, pos: usize) -> Vec<&str> {
    ...
    list.iter().filter_map(|&line| {if line.chars().nth(pos)? == filter {Some(line)} else {None}}).collect()
}
