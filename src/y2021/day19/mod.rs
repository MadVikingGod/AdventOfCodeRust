...
impl Mul<Matrix3<i32>> for Scanner {
    ...
    fn mul(self, rhs: Matrix3<i32>) -> Self::Output {
        let readings = self.readings.iter().map(|v| rhs * v).collect_vec();
        Self {
            rel_coords: self.rel_coords.clone(),
            readings,
        }
    }
}
...
fn get_rot_matrixes() -> Vec<Matrix3<i32>> {
    let rots = vec![
        "I", "X", "Y", "XX", "XY", "YX", "YY", "XXX", "XXY", "XYX", "XYY", "YXX", "YYX", "YYY",
