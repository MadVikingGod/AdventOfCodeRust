...
pub fn group_distance(list: &[i64], pos: i64) -> i64 {
    list.iter().map(|&x| distance(x, pos)).sum()
}

pub fn find_min_distance(list: &[i64]) -> (i64, i64) {
...
pub fn advance_group_distance(list: &[i64], pos: i64) -> i64 {
    list.iter().map(|&x| advance_distance(x, pos)).sum()
}

pub fn find_min_advance_distance(list: &[i64]) -> (i64, i64) {
    let &min = list.iter().min().unwrap();
    let &max = list.iter().max().unwrap();
