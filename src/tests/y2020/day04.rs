use crate::y2020::day04::Passport;
...
// #[test]
// fn test_valid_birth() {
//     assert!(Passport::new("byr:2002".to_string()).valid_birth())
// }
...
eyr:2022",
    ...
    for s in invalid_passports {
        let passport = Passport::new(s.to_string());
        println!("{:?}", passport);
        assert!(!passport.is_valid_extended());
    }
    for s in valid_passports {
        let passport = Passport::new(s.to_string());
        assert!(passport.is_valid_extended());
    }
}
